...
/**
 * Flatten an absolute URL to an app-relative path by stripping the API path.
 */
const flattenURL = (url: string): string => {
...
const getVideoSrc = (url: string) => {
  if (isInternalURL(url)) {
    const flattened = flattenURL(url);
  ...
  let placeholder: string | null =
    typeof data.preview_image === 'string'
      ? isInternalURL(data.preview_image)
        ? `${flattenURL(data.preview_image)}/@@images/image`
        : data.preview_image
      : null;
